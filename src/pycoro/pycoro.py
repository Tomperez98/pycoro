...
from pycoro.aio import AIO, Kind
...
class Pycoro:
    def __init__(self, aio: AIO, size: int, dequeue_size: int, tick_freq: float = 0.1) -> None:
        self._aio: Final = aio
        self._scheduler: Final = Scheduler[Any, Any](self._aio, size)
        self._dequeue_size: Final = dequeue_size
        self._tick_freq: Final = tick_freq
        self._thread: Final = Thread(target=self._loop, daemon=True)
        self._stop: Final = Event()
        self._stopped: Final = Event()
        self._stopped.set()
